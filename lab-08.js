...
fastify.get("/photos", (request, reply) => {
  ...
  fastify.get("/photos/:id", (request, reply) => {
    ...
    .then(response => response.json())
    .then((photos) => {
        Object.keys(photos[id-1]); //Just running this line brings the program to the catch block
        reply
        .code(200)
        .header("Content-Type", "text/json; charset=utf-8")
        .send({ error: "", statusCode: 200, photo: photos[id-1] });
    })
    .catch((error) => {
        const log = Object.keys(error)[0] ? error : "Invalid ID";
        reply
        .code(404)
        .header("Content-Type", "text/json; charset=utf-8")
        .send({log, statusCode: 404, photo: {} });
    });
  });
